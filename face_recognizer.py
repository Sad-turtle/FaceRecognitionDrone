...
while True:
    ...
    faceId, matchConfidence = recognizer.predict(imgGray)  # [startY:endY, startX:endX]
    if matchConfidence <= 80:
        faceName = names[faceId]
        matchConfidence = "  {0}%".format(round(matchConfidence))
        faceBoxColor = (0, 255, 0)
        ...
        faceName = names[0]
        matchConfidence = "  {0}%".format(round(matchConfidence))
        faceBoxColor = (0, 0, 255)

    cv2.rectangle(frame, (startX, startY), (endX, endY), faceBoxColor, 2)
